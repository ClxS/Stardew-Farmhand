...
{    
    var node = document.querySelector('a[href="' + window.location.hash.replace("#", "") + '"]');
    if(node != undefined)
    {
        topicContent.src = node.href;
    }
}

function SetDefaultPageHash() 
{
    UpdatePageHash();
}

function FixupNodeNames() 
{
    var nodes = document.getElementsByClassName('UnselectedNode');
    ...
    for (var i = 0; i < arrayLength; i++) 
    {
        ...
        if(nodes[i].innerHTML.indexOf(" Properties") > 0 || nodes[i].innerHTML.indexOf(" Constructor") > 0|| nodes[i].innerHTML.indexOf(" Methods") > 0 || nodes[i].innerHTML.indexOf(" Fields") > 0) 
        {
            ...
            var visible = false;
            var childItems = parent.parentNode.childNodes;
            var numChildItems = childItems.length;
            for (var j = 0; j < numChildItems; j++) 
            {
                if(childItems[j].style != undefined && childItems[j].style.display != 'none') {
                    visible = true;
                    break;
                }
            }
            ...
            if(!visible) {
                var outerWrapper = parent.parentNode.parentNode;
                outerWrapper.className = 'TreeItem';
                var imgTag = outerWrapper.getElementsByTagName('img')
                imgTag[0].onclick = undefined;
                imgTag[0].src='Item.gif';
                imgTag[0].className = '';
            }
        }
    }

    nodes = document.getElementsByClassName('SelectedNode');
