...
namespace Farmhand.Overrides
{
    ...
    public class Game1 : Farmhand.Overrides.GameOverrideBase
    {
        ...
        {
            if (!options.zoomLevel.Equals(1f))
            {
            ...
            if (gameMode == 6)
            {
                spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.PointClamp, null, null);
                var str = "";
                for (var i = 0; i < ((gameTime.TotalGameTime.TotalMilliseconds%999.0)/333.0); i++)
                    str += ".";
            ...
            else
            {
                if (drawLighting)
                {
                    ...
                    spriteBatch.End();
                    GraphicsDevice.SetRenderTarget(options.zoomLevel.Equals(1f) ? null : screen);
                }
                if (bloomDay)
                {
                    bloom?.BeginDraw();
                ...
                if (eventUp && (currentLocation.currentEvent != null))
                {
                    foreach (NPC npc5 in currentLocation.currentEvent.actors)
                    {
                        if (npc5.isEmoting)
                        {
                            ...
                            position.Y -= (tileSize * 2f) + (pixelZoom * 3f);
                            if (npc5.age == 2)
                            {
