...
namespace TestNPCMod
{
    public class TestNPCMod : Mod
    {
        ...
        private void GameEvents_OnAfterGameLoaded(object sender, EventArgs e)
        {
            Console.WriteLine();
            Console.WriteLine();
            Game1.locations.FirstOrDefault(location => location.Name == "WizardHouse")?.addCharacter(new TestNPC(npcInformation["Troy"]));
            Console.WriteLine(string.Join(", ", Game1.locations.FirstOrDefault(location => location.Name == "WizardHouse")?.getCharacters().Select(npc => npc.getName()) ?? new string[] {"None"}));
            Console.WriteLine();
            Console.WriteLine();
